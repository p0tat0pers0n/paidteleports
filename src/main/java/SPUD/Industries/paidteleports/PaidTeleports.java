...
public final class PaidTeleports extends JavaPlugin implements Listener {
            ...
            if (item != null && item.getItemMeta().hasCustomName() && Objects.requireNonNull(item.getItemMeta().customName()).children().toString().equals(teleportItem)) {
                ...
                for (String commands : tpaTeleport) {
                    ...
                    if (command.equals(commands)) {
                        player.sendMessage("Is a tpa command");
    ...
    public void playerTeleport(PlayerTeleportEvent event) {
        ...
        player.sendMessage("§c§lTpa List:");
        for (UUID test : tpaList.keySet()) {
            player.sendMessage(test.toString() + tpaList.get(test).toString());
        }

        player.sendMessage("§c§lInverse Tpa List:");
        for (UUID test : inverseTpaList.keySet()) {
            player.sendMessage(test.toString() + inverseTpaList.get(test).toString());
        }
